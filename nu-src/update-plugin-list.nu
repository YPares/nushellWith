...
def main [out: path = "plugin-list.toml"] {
  let locked_nu_src_node = open -r ($self | path join .. flake.lock) |
    from json | get -o nodes.nushell-src
  let locked_nu_ref = if $locked_nu_src_node != null {
    print "nushell-src found in flake.lock, reading nushell version from it"
    $locked_nu_src_node.original?.ref? | default "refs/heads/main"
  } else {
    print "nushell-src not found in flake.lock, reading nushell version from nu running this script"
    (version).version
  }
